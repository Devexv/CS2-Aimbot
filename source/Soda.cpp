...
std::mutex mtx;
bool aimbotRunning = false;

void startAimbot() {
    std::lock_guard<std::mutex> lock(mtx);
    std::cout << "Aimbot started." << std::endl;
    aimbotRunning = true;
...
int main() {
    ...
    while (running) {
        std::string input;
        std::cout << "Enter 'start' to start Aimbot or 'stop' to stop Aimbot: ";
        std::cin >> input;
        
        if (input == "start" && !aimbotRunning) {
            aimbotThread = std::thread(startAimbot);
        } else if (input == "stop" && aimbotRunning) {
            stopAimbot();
            if (aimbotThread.joinable()) {
                aimbotThread.join();
            }
        } else {
            std::cout << "Invalid command or Aimbot is already running/stopped." << std::endl;
        }
